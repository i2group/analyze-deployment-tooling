...
# NOTE: this file has a dependency on the common_variables.sh file.
...
POSTGRES_SERVER_VOLUME_NAME="${POSTGRES_SERVER_CONTAINER_NAME}_sqlvolume"
...
POSTGRES_SERVER_BACKUP_VOLUME_NAME="${POSTGRES_SERVER_CONTAINER_NAME}_sqlbackup"
...
PLUGINS_DIR="${ANALYZE_CONTAINERS_ROOT_DIR}/i2a-plugins"
PREVIOUS_PLUGINS_DIR="${PLUGINS_DIR}/.i2a-plugins"
...
if [[ "${ENVIRONMENT}" == "pre-prod" ]]; then
  ...
  LOCAL_ISTORE_NAMES_POSTGRES_PROPERTIES_FILE="${LOCAL_CONFIGURATION_DIR}/fragments/opal-services-is/WEB-INF/classes/InfoStoreNamesPostgres.properties"
  ...
  PREVIOUS_CONFIGURATION_PLUGINS_PATH="${PREVIOUS_CONFIGURATION_DIR}/plugins"
  ...
  if [[ -f "${PREVIOUS_CONFIGURATION_UTILS_PATH}/variables.conf" ]]; then
    source "${PREVIOUS_CONFIGURATION_UTILS_PATH}/variables.conf"
  fi
  PREVIOUS_DEPLOYMENT_PATTERN="${DEPLOYMENT_PATTERN}"
  PREVIOUS_DB_DIALECT="${DB_DIALECT}"
  if [[ -f "${CURRENT_CONFIGURATION_UTILS_PATH}/variables.conf" ]]; then
    source "${CURRENT_CONFIGURATION_UTILS_PATH}/variables.conf"
  fi
  CURRENT_DEPLOYMENT_PATTERN="${DEPLOYMENT_PATTERN}"
  PREVIOUS_STATE_FILE_PATH="${PREVIOUS_CONFIGURATION_DIR}/.state.conf"
fi
